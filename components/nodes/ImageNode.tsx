...
import { NodeType, type ImageNode as ImageNodeType } from '../../types';
  ...
  const handleAnalyze = async (
    ...
    try {
      const description = await analyzeImage(
        node.data,
        'Describe this image for a K-12 student. What is happening here?'
      );
      addNode({
        type: NodeType.Text,
        position: {
          x: node.position.x,
          y: node.position.y + node.size.height + 20,
        },
